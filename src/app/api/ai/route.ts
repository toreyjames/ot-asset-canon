...
// Mission Map Query Tools - Allow AI to query the Mission Map database
const canonQueryTools = {
  searchAssets: tool({
    description:
      "Search for assets in Mission Map by tag number, name, type, layer, or risk tier. Returns asset details with engineering, control system, network, and security context.",
    parameters: z.object({
      query: z.string().optional().describe("Search term for tag number or name"),
      layer: z.number().min(1).max(6).optional().describe("Filter by Mission Map layer (1-6)"),
...
// System prompt for the Mission Map AI
const SYSTEM_PROMPT = `You are the Mission Map AI - an expert system for converged plant intelligence that understands the full context from physical process through enterprise integration.

You have access to:
1. **Mission Map Query Tools** - Search assets, relationships, attack paths, and consequence chains in the plant's unified asset database
...
## Key Capabilities
1. **Consequence Analysis**: Given an asset or attack scenario, trace through Mission Map to identify the full consequence chain - from cyber compromise through physical impact
...
- Reference Mission Map layers (1-6) to show where in the architecture risks exist
